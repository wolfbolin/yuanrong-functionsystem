...
function make_combined_yuanrong_package() {
    ...
    if [[ $(basename "$PREBUILD_BIN_PATH_YUANRONG") == *.tar.gz ]]; then
        log_info "extract prebuild yuanrong.tar.gz to ${COMBINE_OUTPUT_DIR}"
    ...
    if [[ $(basename "$PREBUILD_BIN_PATH_TOOLS") == *.tar.gz ]]; then
        log_info "extract prebuild tools.tar.gz to ${TEMP_BUILD_CACHE_DIR}"
        tar --strip-components=1 -zxf ${PREBUILD_BIN_PATH_TOOLS} -C ${TEMP_BUILD_CACHE_DIR}/prebuild_tools
    else
        log_info "extract prebuild tools to ${TEMP_BUILD_CACHE_DIR}"
        cp -rf ${PREBUILD_BIN_PATH_TOOLS}/* ${TEMP_BUILD_CACHE_DIR}/prebuild_tools/
    fi
