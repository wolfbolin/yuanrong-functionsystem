...
void EtcdServiceDriver::StartServer(const std::string &address, const std::string &prefix)
{
    // stop server before start server.
    StopServer();
    kvActor_ = std::make_shared<meta_store::KvServiceActor>(prefix);
    litebus::AID kvActorAid = litebus::Spawn(kvActor_);

    kvAccessorActor_ = std::make_shared<meta_store::KvServiceAccessorActor>(kvActorAid, prefix);
    litebus::Spawn(kvAccessorActor_);

    leaseActor_ = std::make_shared<meta_store::LeaseServiceActor>(kvActorAid, prefix);
