...
Duration TimeWatch::Now()
{
    ...
    auto duration =
        static_cast<Duration>(ts.tv_sec) * SECTOMILLI + (static_cast<Duration>(ts.tv_nsec) / MICRTONANO) / MILLITOMICR;
    return duration;
}

TimeWatch TimeWatch::In(const Duration &duration)
{
    return TimeWatch(Now() + duration);
}
...
bool TimeWatch::operator==(const TimeWatch &that) const
{
