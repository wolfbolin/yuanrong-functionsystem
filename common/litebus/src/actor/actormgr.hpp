...
#ifndef DEF_ACTOR_MGR_H
#define DEF_ACTOR_MGR_H
...
class ActorMgr {
public:
    static inline std::shared_ptr<ActorMgr> &GetActorMgrRef()
    {
        return actorMgr;
    }
    ...
    static inline std::shared_ptr<IOMgr> &GetIOMgrRef(const AID &to)
    {
        return GetIOMgrRef(to.GetProtocol());
    }
    ...
    inline const std::string &GetDelegate() const
    {
        return delegate;
    }

    inline void SetDelegate(const std::string &d)
    {
        delegate = d;
    }
    inline void SetActorReady(const std::shared_ptr<ActorBase> &actor)
    {
        threadPool.EnqueReadyActor(actor);
    }
