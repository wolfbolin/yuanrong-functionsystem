...
void SingleThread::Terminate(const ActorBase *actor)
{
    ...
    BUSLOG_DEBUG("ACTOR (v={}) SingleThread received terminate message", actorName);
...
ShardedThread::ShardedThread(const std::shared_ptr<ActorBase> &aActor) : ready(false), terminated(false), actor(aActor)
{
}
...
void ShardedThread::Terminate(const ActorBase *aActor)
{
    ...
    BUSLOG_DEBUG("ACTOR (v={}) ShardedThread received terminate message", actorName);
    // remove actor from actorMgr
    ActorMgr::GetActorMgrRef()->RemoveActor(actorName);
